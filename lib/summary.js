...
export const sharePercent = (co2Kg, totalKg) =>
  totalKg > 0 ? Math.round((co2Kg / totalKg) * 1000) / 10 : 0;

// Pure aggregation of activity rows into everything the dashboard and weekly-target cards
// need. `week` is the getWeekInfo() shape. Kept pure so it can be unit-tested and so the
// browser can apply a just-saved activity with the same maths (see lib/optimistic.js).
export function summarize(rows, targetKg, week) {
  ...
  const categories = TYPE_KEYS.map((t) => ({
    ...
    ...all[t],
    share_percent: sharePercent(all[t].co2_kg, totalKg),
  }));
...
export async function buildSummary(supabase) {
  ...
  return summarize(rows, targetKg, getWeekInfo());
}
