import { ACTIVITY_TYPES, TYPE_KEYS } from './constants';
import { round2 } from './co2';
import { buildNudge } from './nudge';
import { getWeekInfo, projectWeekTotal } from './week';
...
export async function buildSummary(supabase) {
  ...
  const projected = projectWeekTotal(weekTotal, week.dayNumber);
  const hasTarget = targetKg != null;
  const exceeded = hasTarget && weekTotal > targetKg;
  ...
  return {
    ...
    week: {
      ...
      total_kg: weekTotal,
      target_kg: targetKg,
      percent: hasTarget ? Math.round((weekTotal / targetKg) * 1000) / 10 : null,
      remaining_kg: hasTarget ? round2(Math.max(targetKg - weekTotal, 0)) : null,
      exceeded,
      exceeded_by_kg: exceeded ? round2(weekTotal - targetKg) : 0,
      projected_kg: projected,
      projected_exceeds: hasTarget && !exceeded && projected > targetKg,
      nudge: exceeded ? buildNudge(thisWeek) : null,
    },
  };
}
